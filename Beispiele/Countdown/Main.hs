...
update Add10Seconds model@(Setting secs) =
  model { setSeconds = secs + 10 }
update (StartStop now) (Setting secs) =
  Running $ Clock.addUTCTime (fromIntegral secs) now
